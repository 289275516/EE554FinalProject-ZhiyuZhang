...
yolo_dir = r'C:\Users\28927\Desktop\EE555finalversion'  # YOLO文件路径
weightsPath = os.path.join(yolo_dir, 'my_yolov3.backup')  # 权重文件
configPath = os.path.join(yolo_dir, 'yolov3.cfg')  # 配置文件
labelsPath = os.path.join(yolo_dir, 'myData.names')  # label名称
...
if len(idxs) > 0:
    for i in idxs.flatten(): # indxs是二维的，第0维是输出层，所以这里把它展平成1维
        ...
        if str(classIDs[i]) == '1':
            ...
            print(w)
            print(h)
            cv.rectangle(img,(int((x+w/2)), int((y+h/2))),(int((x+w/2)), int((y+h/2))),shootcolor, 2)
            shootlocationx = int(x+w/2)
            shootlocationy = int(y+h/2)
...
pyautogui.moveTo(shootlocationx, shootlocationy, duration=1)
pyautogui.click()
cv.waitKey(0)
